...
const languageVariantsDecorator = require('./decorators/languageVariantsDecorator');

const typeItemDecorator = require('./decorators/typeItemDecorator');

const linkedItemsElementDecorator = require('./decorators/linkedItemsElementDecorator');

const richTextElementDecorator = require('./decorators/richTextElementDecorator');
...
exports.sourceNodes = async (api, {
 ...
 ${_.get(deliveryClientConfig, 'projectId')}`);
  ...
  if (lastModified && types !== []) {
    ...
    const kenticoCloudNodes = getNodes().filter(item => item.internal.owner === 'gatsby-source-kentico-cloud' // TODO  Extract constant to one place and use it from there and normalize
    && item.internal.type.startsWith(`KenticoCloudItem`));
    const client = new DeliveryClient(deliveryClientConfig);
    const itemsToUpdate = await client.items().greaterThanFilter(`system.last_modified`, lastModified.toISOString()).getPromise();
    ...
    richTextElementDecorator.resolveHtml(itemsToUpdate.items);
    const resolvedItemsToUpdate = parse(stringify(itemsToUpdate.items));

    for (const node of kenticoCloudNodes) {
      const updateItem = resolvedItemsToUpdate.find(item => // TODO use Gatsby Node ID for filtering
      item.system.codename === node.system.codename && item.system.language === node.system.language);

      if (updateItem) {
        // update
        // TODO come up with different approach for updating and decorating! items unify with decorators code
        const itemNode = itemNodes.createContentItemNode(createNodeId, updateItem, types); // TODO extract additional data to the constants from typeNodes and ItemNodes to constants

        itemNode.otherLanguages___NODE = node.otherLanguages___NODE;
        itemNode.contentType___NODE = node.contentType___NODE; // TODO Add linked items from itemNode.elements.<element>___NODE  = node.elements.<element>___NODE
  ...
  const defaultCultureContentItemNodes = await itemNodes.getFromDefaultLanguage(client, defaultLanguageCodename, contentTypeNodes, createNodeId);
  const nonDefaultLanguageItemNodes = await itemNodes.getFromNonDefaultLanguage(client, nonDefaultLanguageCodenames, contentTypeNodes, createNodeId);
  languageVariantsDecorator.decorateItemsWithLanguageVariants(defaultCultureContentItemNodes, nonDefaultLanguageItemNodes);
  const allItemNodes = defaultCultureContentItemNodes.concat(_.flatten(nonDefaultLanguageItemNodes.values()));
  typeItemDecorator.decorateTypeNodesWithItemLinks(allItemNodes, contentTypeNodes);
  linkedItemsElementDecorator.decorateItemNodesWithLinkedItemsLinks(defaultCultureContentItemNodes, nonDefaultLanguageItemNodes);
  richTextElementDecorator.decorateItemNodesWithRichTextLinkedItemsLinks(defaultCultureContentItemNodes, nonDefaultLanguageItemNodes);
  console.info(`Creating content type nodes.`);
...
const addHeader = (deliveryClientConfig, trackingHeader) => {
  let headers = deliveryClientConfig.globalHeaders ? _.cloneDeep(deliveryClientConfig.globalHeaders) : [];
