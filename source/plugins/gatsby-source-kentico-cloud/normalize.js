...
const parseContentItemContents = (contentItem, processedContents = []) => {
  ...
  processedContents.push(contentItem.system.codename);
  const elements = {};
  const elementPropertyKeys = Object.keys(contentItem).filter(key => key !== `system` && key !== `elements`);
  ...
  for (const key of elementPropertyKeys) {
    ...
    if (_.get(contentItem, `elements[${key}].type`) === 'modular_content') {
      ...
      contentItem[key].forEach(linkedItem => {
        linkedItems.push(parseContentItemContents(linkedItem, Array.from(processedContents), contentItem));
      });
...
const createKcArtifactNode = (nodeId, kcArtifact, artifactKind, codeName = ``, additionalNodeData = null) => {
...
const addLinkedItemsLinks = (itemNode, linkedNodes, linkPropertyName, originalNodeCollection = []) => {
  ...
  const sortPattern = originalNodeCollection.map(item => item.system.id);
  const sortedLinkedNodes = linkedNodes.sort((a, b) => sortPattern.indexOf(a.system.id) - sortPattern.indexOf(b.system.id)).map(item => item.id);
