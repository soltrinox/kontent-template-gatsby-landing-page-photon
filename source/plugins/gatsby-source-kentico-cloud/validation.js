...
const checkItemsObjectStructure = contentItemNodes => {
  if (!hasBasicValidStructure(contentItemNodes) && _.every(contentItemNodes, item => _.has(item, 'system.language') && _.has(item, 'system.type'))) {
    throw new Error(`contentItemNodes is not an array of valid objects.`);
  }
};

const hasBasicValidStructure = contentNodes => {
  return _.isArray(contentNodes) && _.every(contentNodes, item => _.has(item, 'system.id') && _.has(item, 'system.name') && _.has(item, 'system.codename') && _.has(item, 'elements'));
};
