...
const decorateItemNodesWithRichTextLinkedItemsLinks = (defaultCultureContentItemNodes, nonDefaultLanguageItemNodes) => {
  defaultCultureContentItemNodes.forEach(itemNode => {
    try {
      decorateItemNodeWithRichTextLinkedItemsLinks(itemNode, defaultCultureContentItemNodes);
    } catch (error) {
      console.error(error);
    }
  });
  nonDefaultLanguageItemNodes.forEach(languageNodes => {
    languageNodes.forEach(itemNode => {
      try {
        decorateItemNodeWithRichTextLinkedItemsLinks(itemNode, languageNodes);
      } catch (error) {
        console.error(error);
      }
    });
...
const decorateItemNodeWithRichTextLinkedItemsLinks = (itemNode, allNodesOfSameLanguage) => {
  validation.checkItemsObjectStructure([itemNode]);
  validation.checkItemsObjectStructure(allNodesOfSameLanguage);
  Object.keys(itemNode.elements).forEach(propertyName => {
    ...
    if (_.get(property, `type`) === `rich_text`) {
      ...
      const linkedNodes = allNodesOfSameLanguage.filter(node => _.has(property, `linkedItemCodenames`) && _.isArray(property.linkedItemCodenames) && property.linkedItemCodenames.includes(node.system.codename));

      _.set(itemNode.elements, linkPropertyName, []);

      normalize.addLinkedItemsLinks(itemNode, linkedNodes, linkPropertyName);
    }
  });
};
