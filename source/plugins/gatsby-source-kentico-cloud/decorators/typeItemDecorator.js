...
const decorateTypeNodesWithItemLinks = (contentItemNodes, contentTypeNodes) => {
  ...
  contentTypeNodes.forEach(contentTypeNode => {
    const itemNodesPerType = contentItemNodes.filter(contentItemNode => contentItemNode.system.type === contentTypeNode.system.codename);

    if (!_.isEmpty(itemNodesPerType)) {
      const flatList = itemNodesPerType.map(itemNodePerType => itemNodePerType.id);
