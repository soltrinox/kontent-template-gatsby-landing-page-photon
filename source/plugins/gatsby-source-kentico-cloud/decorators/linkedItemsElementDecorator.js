...
const decorateItemNodesWithLinkedItemsLinks = (defaultCultureContentItemNodes, nonDefaultLanguageItemNodes) => {
  defaultCultureContentItemNodes.forEach(itemNode => {
    try {
      decorateItemNodeWithLinkedItemsLinks(itemNode, defaultCultureContentItemNodes);
    } catch (error) {
      console.error(error);
    }
  });
...
const decorateItemNodeWithLinkedItemsLinks = (itemNode, allNodesOfSameLanguage) => {
  ...
  Object.keys(itemNode.elements).forEach(propertyName => {
    ...
    if (_.isArray(property)) {
      ...
      if (_.has(property, `[0].system.codename`)) {
        const linkedNodes = allNodesOfSameLanguage.filter(node => {
          const match = property.find(propertyValue => {
            return propertyValue !== null && node !== null && propertyValue.system.codename === node.system.codename && propertyValue.system.type === node.system.type;
          });
          return match !== undefined && match !== null;
        });
        normalize.addLinkedItemsLinks(itemNode, linkedNodes, linkPropertyName, itemNode.elements[propertyName]);
      }
