...
const getFromDefaultLanguage = async (client, defaultLanguageCodename, contentTypeNodes, createNodeId) => {
  const contentItemsResponse = await client.items().languageParameter(defaultLanguageCodename).getPromise();

  const allItems = _.unionBy(contentItemsResponse.items, contentItemsResponse.linkedItems, 'system.codename');
...
const getFromNonDefaultLanguage = async (client, nonDefaultLanguageCodenames, contentTypeNodes, createNodeId) => {
  ...
  for (const languageCodename of nonDefaultLanguageCodenames) {
    ...
    const contentItemsNodes = languageItemsFlatted.map(languageItem => createContentItemNode(createNodeId, languageItem, contentTypeNodes));
